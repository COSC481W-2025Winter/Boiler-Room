...
export default function GameRecommendation() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      ...
      if (minYear.length != 4 || maxYear.length != 4) {
        errors += 'Minimum/Maximum year is incorrect length - YYYY format.\n'
      }
      if (minYear < 1980 || maxYear < 1980) {
        errors += 'Minimum/Maximum year is too old - min = 1980.\n'
      }
      if (minYear > new Date().getFullYear() || maxYear > new Date().getFullYear()) {
        errors += 'Minimum/Maximum year is invalid - max year = current year.\n'
      }
      if (minYear > maxYear) {
        let swap = minYear
        minYear = maxYear
      ...
      if (maxHLTB < 0 || maxHLTB > 100) {
        errors += 'How Long to beat score invalid - 1 to 100.\n'
      }
